...
public class PojoDTO {
    ...
    public static boolean checkInclude(List<PojoDTO> listBean, Object sValue) {
        if (sValue != null && listBean != null && !listBean.isEmpty()) {
            for (int i = 0; i < listBean.size(); i++) {
                if (listBean.get(i).getValue() != null && listBean.get(i).getValue().toString().equals(sValue.toString())) {
                    return true;
                }
            }
        }
    ...
    public static boolean checkIncludeKey(List<PojoDTO> listBean, Object sValue) {
        if (sValue != null && listBean != null && !listBean.isEmpty()) {
            for (int i = 0; i < listBean.size(); i++) {
                if (listBean.get(i).getKey() != null && listBean.get(i).getKey().toString().equals(sValue.toString())) {
                    return true;
                }
            }
        }
